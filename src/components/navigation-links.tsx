    ...
    <nav className="flex flex-wrap justify-center gap-4 md:gap-6 px-4">
      {links.map((link) => (
        <Button key={link.name} variant="ghost" size="lg" asChild className="group !p-0">
          <a href={link.href} className="flex flex-col items-center space-y-2 p-3 rounded-lg hover:bg-accent/70 transition-all duration-300 ease-in-out transform hover:scale-105 w-28 h-28 justify-center">
